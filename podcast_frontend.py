...
def main():
    ...
    st.markdown(
        f"<h1 style='color: white;'>Podcast Dashboard</h1>", unsafe_allow_html=True
    )
    ...
    search_query = st.sidebar.text_input(
        "Suggestions: XAI, Biden, Pizza, etc.",
    ...
    selected_podcast = st.sidebar.selectbox(
        ...
        index=get_selected_podcast_index(
            st.session_state.selected_podcast, available_podcast_info
        ),
    )
    ...
    if selected_podcast:
        ...
        st.markdown(
            f"<h2 style='color: #999afd;'>Podcast Content</h2>", unsafe_allow_html=True
        )

        # Display the podcast title
        # st.subheader("Episode Title")
        st.markdown(
            f"<h3 style='color: #f582ff;'>Episode Title</h3>", unsafe_allow_html=True
        )
        st.markdown(
        ...
        with col1:
            ...
            st.markdown(
                f"<h3 style='color: #f582ff;'>Podcast Episode Summary</h3>",
                unsafe_allow_html=True,
            )
        ...
        with col3:
            ...
            st.markdown(
                f"<h3 style='color: #f582ff;'>Podcast Guest</h3>",
                unsafe_allow_html=True,
            )
        ...
        with col4:
            ...
            st.markdown(
                f"<h3 style='color: #f582ff;'>Podcast Guest Details</h3>",
                unsafe_allow_html=True,
            )
        ...
        st.markdown(
            f"<h3 style='color: #f582ff;'>Key Moments</h3>", unsafe_allow_html=True
        )
        key_moments = podcast_info["podcast_highlights"]
        key_highlights = podcast_info["podcast_highlights"].split("\n")

        st.markdown(
            f"<p style='margin-bottom: 15px; color: #FFDB58;'><i>Key highlights from the podcast transcription are:</i></p>",
            unsafe_allow_html=True,
        )
        ...
        st.markdown(
            f"<h3 style='color: #f582ff;'>Key Moments Timeline</h3>",
            unsafe_allow_html=True,
        )

        # add a subheader, small in font-size and in italics
        st.markdown(
            f"<p style='font-size: small; margin-bottom: 20px; color: #FFDB58;font-style: italic;'>Coming soon! The chart below shows the time distribution of key moments in the podcast episode.</p>",
            unsafe_allow_html=True,
        )
        timeline_data = pd.DataFrame(
            {"Moment": key_highlights, "Time": range(1, len(key_highlights) + 1)}
        )
        chart = (
    ...
    url = st.sidebar.text_input(
        "Link to RSS Feed", placeholder="Enter the RSS Feed URL here..."
    )
    ...
    st.sidebar.markdown("---")
    st.sidebar.markdown(
        "<p style='font-size: small; font-style: italic; margin-top: auto;'>Created by: <a href='https://sankalp-prabhakar.github.io/' target='_blank' style='color: pink;'>Sankalp Prabhakar</a></p>",
        unsafe_allow_html=True,
    )
...
def find_matching_podcasts(query, podcast_info_dict):
    ...
    for podcast_name, podcast_info in podcast_info_dict.items():
        if (
            query.lower() in podcast_name.lower()
            or query.lower() in podcast_info["podcast_summary"].lower()
        ):
